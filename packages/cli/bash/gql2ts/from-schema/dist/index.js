...
const run = (schemaInput, optionsInput) => {
  ...
  */`
  ...
  const fieldToDefinition = (field, isInput, supportsNullability) => {
    ...
    const isNotNull = interfaceName.includes('!');
    const showNullabiltyAttribute = !isNotNull && supportsNullability;
    if (isNotNull) {
      /**
  ...
  const typeToInterface = (
    ...
    if (isUnion(type)) {
      return `export ${generateAbstractTypeDeclaration(
        type,
        ignoredTypes,
        interfaceMap
      )}`;
    ...
    if (util_1.isEnum(type)) {
      return `export ${generateEnumDeclaration(
        type.description,
        type.name,
        type.getValues()
      )}`;
    ...
    const fields = f
      ...
      .map((field) => [
        ...
        fieldToDefinition(field, isInput, supportsNullability),
      ])
    ...
    if (graphql_1.isAbstractType(type)) {
      additionalInfo = generateAbstractTypeDeclaration(
        type,
        ignoredTypes,
        interfaceMap
      );
  ...
  const typesToInterfaces = (schema, options) => {
    ...
    typeInterfaces.forEach((type) => {
      ...
      typesWithExport.push(
        typeInterface.includes('?:')
          ? typeInterface
          : typeInterface.replace(/:/g, '?:')
      );
    });
    return interfaces
...
exports.schemaToInterfaces = (schema, options = {}, formatters = {}) =>
  run(
    util_1.schemaFromInputs(schema),
    Object.assign({}, options, {
      formats: Object.assign(
        {},
        language_typescript_1.DEFAULT_OPTIONS,
        formatters
      ),
    })
  );
exports.generateNamespace = (
  namespace,
  schema,
  options = {},
  overrides = {}
  ...
  return formatters.postProcessor(
    formatters.generateNamespace(
      namespace,
      exports.schemaToInterfaces(schema, options, formatters)
    )
  );
