...
exports.ADD_INTERFACE_EXTENSIONS = (opName, exts) =>
  opName + exports.interfaceExtendListToString(exts);
exports.DEFAULT_NAME_FRAGMENT = name => `IFragment${humps_1.pascalize(name)}`;
exports.DEFAULT_NAME_QUERY = def =>
  def.name ? humps_1.pascalize(def.name.value) : 'Anonymous';
exports.DEFAULT_FORMAT_INPUT = (name, isOptional, type) =>
  exports.ADD_SEMICOLON(`${name}${isOptional ? '?:' : ':'} ${type}`);
...
exports.DEFAULT_TYPE_PRINTER = (type, isNonNull) =>
  isNonNull ? type : `${type} | null`;
exports.DEFAULT_GENERATE_SUBTYPE_INTERFACE_NAME = selectionName =>
  `SelectionOn${humps_1.pascalize(selectionName)}`;
exports.DEFAULT_ENUM_FORMATTER = values =>
  values.map(v => `'${v.value}'`).join(' | ');
exports.DEFAULT_ENUM_NAME_GENERATOR = name => `I${humps_1.pascalize(name)}Enum`;
exports.DEFAULT_INPUT_NAME_GENERATOR = name =>
  `${humps_1.pascalize(name)}Input`;
exports.DEFAULT_EXPORT_FUNCTION = declaration => `export ${declaration}`;
exports.ADD_SEMICOLON = str => `${str};`;
exports.DEFAULT_NAMESPACE_GENERATOR = (
  namespaceName,
  interfaces
