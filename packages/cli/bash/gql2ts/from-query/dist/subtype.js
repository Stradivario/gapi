...
exports.GenerateSubtypeCache = () => {
  ...
  const subTypeCacheHit = (name, declaration) =>
  ...
  const enumerateSubtypes = (name, declaration) => {
    ...
    let i = 1;
    while (true) {
      const tempName = generateEnumeratedName(name, i);
      if (
        GeneratedSubtypes.has(tempName) &&
        !subTypeCacheHit(tempName, declaration)
      ) {
        i++;
        continue;
      } else {
  ...
  const getSubtype = (selection, declaration, generateSubTypeInterfaceName) =>
    subTypeStuff(
      generateSubTypeInterfaceName(selection.name.value, selection),
      declaration
    );
  return getSubtype;
};
